package exercise.ap_1;

import java.util.List; //needed for wordsWithoutList 
import java.util.ArrayList; //needed for wordsWithoutList 
...
class AP_1 {
    ...
    public boolean scoresClump(int[] scores) {
        ...
        for (int i = 0; i < scores.length - 2; i++) {
            difA = scores[i+1] - scores[i];
            difB = scores[i+2] - scores[i];
            if (difA < 3 && difB < 3) return true;
        }
        return false;
    }
    ...
    public int scoresAverage(int[] scores) {
        int aL = average(scores, 0, scores.length/2);
        int aR = average(scores, scores.length/2, scores.length);
        return (aL > aR) ? aL : aR;
    }
    ...
    public int[] copyEvens(int[] nums, int count) {
        ...
        int index = 0;
        for (int i = 0; index < count && i < nums.length; i++) {
            if (nums[i] % 2 == 0) arr[index++] = nums[i];
        }
        return arr;
    }
    ...
    public int[] copyEndy(int[] nums, int count) {
        ...
        int index = 0;
        for (int i = 0; index < count && i < nums.length; i++) {
            if (isEndy(nums[i])) arr[index++] = nums[i];
        }
        return arr;
    }
      
    boolean isEndy(int n) {
        return n > -1 && n < 11 || n > 89 && n < 101;
    }
    ...
    public int matchUp(String[] a, String[] b) {
        ...
        for (int i = 0; i < a.length; i++) {
            ...
            if (a[i].charAt(0) == b[i].charAt(0)) cnt++;
        }
        return cnt;
    }
    ...
    public String[] wordsWithout(String[] words, String target) {
        ...
        int index = 0;
        for (int i = 0; i < words.length && index < cnt; i++) {
            if (!words[i].equals(target)) arr[index++] = words[i];
        }
        return arr;
    }
     ...
     * (the value is not limited to -1/0/1). (On the AP, there would be two User
    ...
    public String[] mergeTwo(String[] a, String[] b, int n) {
        ...
        int indB = 0;
        for (int i = 0; i < n; i++) {
            int comp = a[indA].compareTo(b[indB]);
            if (comp == 0) {
                arr[i] = a[indA++];
                indB++; 
                continue;
            }
            arr[i] = (comp < 0) ? a[indA++] : b[indB++];
        }
        return arr;
    }
